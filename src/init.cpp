...
static void StartStakingThread(NodeContext& node)
{
    ...
    g_staking_thread = std::thread([wallet = wallets[0], &node]() {
        ...
        PoSMiner(wallet, *Assert(node.connman), *Assert(node.llmq_ctx), *Assert(node.evodb), Assert(node.chainman)->ActiveChainstate(), *Assert(node.mempool), *Assert(g_staking_interrupt));
    });
}
#endif // ENABLE_WALLET
